...
DEFAULT_OUTPUT_CSV = PROCESSED_DATA_DIR / "round_predictions_neural.csv"


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Predict T-side round win probabilities with the neural model."
    )
    parser.add_argument(
        "--dataset-csv",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Path to the trained neural model checkpoint.",
    )
    parser.add_argument(
        "--output-csv",
        type=Path,
...
def main(argv: list[str] | None = None) -> int:
    ...
    predictions = predict_round_probabilities_neural(
        dataset,
    ...
    print(f"Saved neural predictions to {args.output_csv}")
    return 0
